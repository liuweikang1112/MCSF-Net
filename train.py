...
from model import MCSFNet
from metrics import DiceLoss, DiceBCELoss, BceLoss
...
def train(model, loader, optimizer, loss_fn, weight_loss, device):
    model.train()
    ...
    for i, (x, y) in enumerate(loader):
        ...
        optimizer.zero_grad()
        y_pred, y_shape = model(x)
        loss = loss_fn(y_pred, y) + weight_loss(y_shape, y)
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    transform = A.Compose([
        ...
        A.CoarseDropout(p=0.5, max_holes=10, max_height=32, max_width=32)
    ])
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=20, eta_min=0.00001, verbose=True)
    loss_fn = DiceBCELoss()
    weight_loss = BceLoss()
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_metrics = train(model, train_loader, optimizer, loss_fn, weight_loss, device)
        valid_loss, valid_metrics = evaluate(model, valid_loader, loss_fn, device)

        if valid_metrics[1] > best_valid_metrics:
            data_str = f"Valid F1 improved from {best_valid_metrics:2.4f} to {valid_metrics[1]:2.4f}. Saving checkpoints: {checkpoint_path}"
            print_and_save(train_log_path, data_str)
        ...
        data_str += f"\tTrain Loss: {train_loss:.4f} - Jaccard: {train_metrics[0]:.4f} - F1-score(Dice): {train_metrics[1]:.4f} - Recall: {train_metrics[2]:.4f} - Precision: {train_metrics[3]:.4f}\n"
